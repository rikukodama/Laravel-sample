...
class Common extends Controller
{
    ...
    public function paginate($searchColumnAry = null, $joins = null, $order = null, $add_condition = null, $controller = null, $groupBy = null)
    {
        ...
        if ($this->_controller != null) {
            ...
            // Continue with the remaining operations involving modelClass
        } else {
            // Handle the scenario where $this->_controller is still null
            // Error logging, error messages, or additional recovery steps can be implemented here
        }
        // パラメータを Array に設定
        $conditionAry = [];
        $conditions = [];
        ...
        $this->_controller->set('list', $this->_controller->paginate(null, [], [], $maxcount));

        $limit = Configure::read('Paginate.LinesPerPage');
        $count = count($this->_controller->paginate(null, [], [], $maxcount));
        ...
        $param['data'] = $this->_controller->paginate(null, [], [], $maxcount);
